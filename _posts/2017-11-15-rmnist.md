...
introductory note, the notes aren't at all complete, and I'm not
thoroughly familiar with prior work. Rather, this is me getting
familiar with the problem by doing some basic hands-on work.
(Frankly, I also wanted to experiment with the scikit-learn and
pytorch libraries!)
...
When I say MNIST, I mean the full set of images.  Here are the digits
in RMNIST/1:
...
RMNIST/10. Still, we're a long way short of where we'd like to be.
...
alone.  That unfairly disadvantages the computer when compared to a
human, since human beings don't learn to recognize new image classes
from scratch.  Rather, they take advantage of what their minds have
already learnt about vision, both from experience and from evolution.
...
has thus learnt from an enormous amount about how to classify images
in general, but not about RMNIST in particular.
...
features.  The classifier we use is a neural network with a single
hidden layer containing 300 neurons.  Here are the results:
...
we obtained using convolutional networks.  At least with this approach
it was more efficient to build a special-purpose convolutional
network, rather than using the features learnt by a convolutional
network trained on a much more general task.

Suppose we use transfer learning based on the algorithmically expanded
data sets.  In that case the results get a little better, but still
don't do as well as the convolutional networks trained without the
help of additional data:
...
less systematic experiments using pytorch and (presumably)
better-designed network architectures, and got to roughly 90% on
RMNIST/10. It'd be good to investigate this more systematically.
...
by doing more exploration using obvious techniques.
...
These would be true super-trainers, essentially canonical examples
from which to learn.
